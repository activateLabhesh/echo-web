...
export class VoiceErrorHandler {
  private static errorMap: Record<string, VoiceError> = {
    ...
    INIT_FAILED: {
      code: "INIT_FAILED",
      message:
        "Failed to initialize voice/video. Please refresh and try again.",
      severity: "high",
      action: "refresh_page",
      retryable: true,
    },
    ...
    NETWORK_QUALITY_POOR: {
      ...
      message:
        "Poor network quality detected. Consider lowering video quality.",
      severity: "low",
    ...
    BROWSER_NOT_SUPPORTED: {
      ...
      message:
        "Your browser does not support voice/video calls. Please use a modern browser.",
      severity: "critical",
    ...
    switch (error.name) {
      ...
      case "OverconstrainedError":
        return {
          ...
          message:
            "Device constraints could not be satisfied. Please check your camera/microphone settings.",
          severity: "medium",
  ...
  static getActionMessage(action: string): string {
    const actionMessages: Record<string, string> = {
      ...
      check_devices:
        "Please check that your camera and microphone are connected and working.",
      connect_device: "Please connect a camera and microphone to continue.",
      grant_permissions:
        "Please allow camera and microphone access in your browser settings.",
      close_other_apps:
        "Please close other applications using your camera or microphone.",
      ...
      grant_recording_permission:
        "Please allow recording permissions in your browser.",
      refresh_page: "Please refresh the page to reconnect.",
      retry_later: "Please try again in a few minutes.",
      wait_and_retry:
        "Please wait for someone to leave the channel and try again.",
      update_browser:
      ...
      check_device_settings:
        "Please check your camera and microphone settings.",
...
export const handleVoiceError = (
  ...
  console.error(
    `Voice Error [${processedError.code}]:`,
